...
class progressBar:
    ...
    def print_progress(self, cur, total, start):
        ...
        bar = '\r[' + '='.join(['' for _ in range(curbar)])  # Drawing Progress
        bar += '>'
        bar += ' '.join(['' for _ in range(int(self.barlength - curbar))]) + '] '  
        bar += bytestostr(cur / elapsed) + '/s '  # Calculates Rate
        bar += getHumanTime((total - cur) * (elapsed / cur)) + ' left'  # Calculates time
        if len(bar) > self.longest:  
            self.longest = len(bar)
            bar += ' '.join(['' for _ in range(self.longest - len(bar))])
        sys.stdout.write(bar)

    def print_end(self, *args):  # Clear Progress Bar
...
def getPlaylistVideoUrls(page_content, url):
    print("WELCOME TO GITSALAHYT DOWNLOADER")
...
def download_Video_Audio(path, vid_url, file_no):
    ...
    try:
        bar = progressBar()
        video.download(path)
        print("successfully downloaded BY GITSALAHYT", yt.title, "!")
    except OSError:
...
if __name__ == '__main__':
    if len(sys.argv) < 2 or len(sys.argv) > 3:
        print('USAGE GITSALAHYT: py main.py playlistURL OR py main.py playlistURL destPath')
        exit(1)
    else:
        url = sys.argv[1]
